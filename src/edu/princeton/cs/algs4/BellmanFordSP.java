...
/*************************************************************************
 * Compilation: javac BellmanFordSP.java Execution: java BellmanFordSP
 * filename.txt s Dependencies: EdgeWeightedDigraph.java DirectedEdge.java
 * Queue.java EdgeWeightedDirectedCycle.java Data files:
		...
		while (!queue.isEmpty() && !hasNegativeCycle()) {
			...
			for (int x = 0; x < G.V(); x++)
				if (edgeTo[x] == null)
					...
					StdOut.println(x + " " + edgeTo[x].from() + "->"
							+ edgeTo[x].to() + " " + distTo[x]);
			StdOut.println();
